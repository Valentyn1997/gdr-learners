...
class HCMNISTSubset(datasets.MNIST):
    ...
    def get_data(self) -> dict:
        ...
        out_scaler.fit(self.y)

        out_f_scaled = out_scaler.transform(self.y.reshape(-1, 1)).reshape(-1)
        out_pot0_scaled = out_scaler.transform(self.y0.reshape(-1, 1)).reshape(-1)
        out_pot1_scaled = out_scaler.transform(self.y1.reshape(-1, 1)).reshape(-1)
        ...
        return {
            ...
            'out_pot0': self.y0,
            'out_pot1': self.y1,
            'out_pot0_scaled': out_pot0_scaled,
            'out_pot1_scaled': out_pot1_scaled,
